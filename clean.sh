...
for f in c00_greetings_d_wsl_ubuntu_template c00_greetings_d_wsl_ubuntu_template.exe c00_greetings_d_wsl_ubuntu_template.o c00_greetings_d_wsl_ubuntu_template.obj c00_greetings_d_wsl_ubuntu_template.pdb; do
    ...
    # Remove from .dub/debug if present
    if [ -e ".dub/debug/$f" ]; then
        echo "Removing .dub/debug/$f..."
        rm -f ".dub/debug/$f"
    fi
    # Remove from .dub/release if present
    if [ -e ".dub/release/$f" ]; then
        echo "Removing .dub/release/$f..."
        rm -f ".dub/release/$f"
    fi
    # Remove from .dub/test if present
    if [ -e ".dub/test/$f" ]; then
        echo "Removing .dub/test/$f..."
        rm -f ".dub/test/$f"
    fi
    # Remove from .dub/objects if present
    if [ -e ".dub/objects/$f" ]; then
        echo "Removing .dub/objects/$f..."
        rm -f ".dub/objects/$f"
    fi
    # Remove from .dub/ if present
    if [ -e ".dub/$f" ]; then
